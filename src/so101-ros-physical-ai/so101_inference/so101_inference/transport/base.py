...
try:
    ...
    # LeRobot is intentionally provided via the Pixi `lerobot` environment.
    # Keep the ROS package importable (e.g. for ament lint tests) even when the
    # user hasn't installed LeRobot into the system Python.
    RemotePolicyConfig = object  # type: ignore[assignment]
    TimedAction = object  # type: ignore[assignment]
    TimedObservation = object  # type: ignore[assignment]
