...
class Board:
    ...
    def bus_servo_read_and_unpack(self, servo_id, cmd, unpack, timeout=None):
        with self.servo_read_lock:
            # Drop any stale response so a previous late packet doesn't get mis-associated.
            try:
                while True:
            ...
            while True:
                try:
                    if end_time is None:
                        # For backward compatibility, don't specify timeout if not provided
                        data = self.bus_servo_queue.get(block=True)
                    else:
                        remaining = end_time - time.time()
                        if remaining <= 0.0:
                            return None
                        data = self.bus_servo_queue.get(block=True, timeout=remaining)
                except queue.Empty:
                    return None

                resp_servo_id, resp_cmd, success, *info = struct.unpack(unpack, data)
                # Some calls use broadcast servo_id=254; in that case accept any responder.
                if servo_id != 254 and resp_servo_id != servo_id:
                    continue
                if resp_cmd != cmd:
                    continue
                if success == 0:
    ...
    def bus_servo_read_id(self, servo_id=254, timeout=None):
        return self.bus_servo_read_and_unpack(servo_id, 0x12, "<BBbB", timeout=timeout)

    def bus_servo_read_offset(self, servo_id, timeout=None):
        return self.bus_servo_read_and_unpack(servo_id, 0x22, "<BBbb", timeout=timeout)
    
    def bus_servo_read_position(self, servo_id, timeout=None):
        return self.bus_servo_read_and_unpack(servo_id, 0x05, "<BBbh", timeout=timeout)

    def bus_servo_read_vin(self, servo_id, timeout=None):
        return self.bus_servo_read_and_unpack(servo_id, 0x07, "<BBbH", timeout=timeout)
    
    def bus_servo_read_temp(self, servo_id, timeout=None):
        return self.bus_servo_read_and_unpack(servo_id, 0x09, "<BBbB", timeout=timeout)

    def bus_servo_read_temp_limit(self, servo_id, timeout=None):
        return self.bus_servo_read_and_unpack(servo_id, 0x3A, "<BBbB", timeout=timeout)

    def bus_servo_read_angle_limit(self, servo_id, timeout=None):
        return self.bus_servo_read_and_unpack(servo_id, 0x32, "<BBb2H", timeout=timeout)

    def bus_servo_read_vin_limit(self, servo_id, timeout=None):
        return self.bus_servo_read_and_unpack(servo_id, 0x36, "<BBb2H", timeout=timeout)

    def bus_servo_read_torque_state(self, servo_id, timeout=None):
        return self.bus_servo_read_and_unpack(servo_id, 0x0D, "<BBbb", timeout=timeout)
